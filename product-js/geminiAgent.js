...
export async function runGeminiWithMcp({ message, finalQuery = "", customerId, jobId, conversationHistory = [], onPhase }) {
  ...
  const systemPrefix =
    ...
    "For root-cause dip questions, answer in a diagnostic format: executive read, primary cause, evidence, what went right, what went wrong, recent change signals, impression-share clues when available, immediate actions, and what not to assume. Do not fall back to generic recommendations alone. " +
