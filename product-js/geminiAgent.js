...
export async function runGeminiWithMcp({ message, finalQuery = "", customerId, jobId, conversationHistory = [], onPhase }) {
  ...
  const systemPrefix =
    ...
    "For decline or root-cause questions, prioritize traffic, conversion rate, budget, bid, status, and recent change-history signals before concluding. Use direct, decisive language when the evidence is strong. Do not claim a policy or verification block unless a tool result or change event supports it. " +
    "For root-cause dip questions, answer in a diagnostic format: executive read, primary cause, evidence, what went right, what went wrong, recent change signals, impression-share clues when available, immediate actions, and confidence. Do not fall back to generic recommendations alone. Never tell the user to refer to a previous response; restate the relevant facts in the current answer. " +
...
export async function runGeminiUnderstanding({ message, customerId, conversationHistory = [] }) {
  ...
  const prompt = [
    ...
    "Never tell the user to refer to a previous response. If prior context matters, restate the relevant facts in the current answer in one short sentence.",
