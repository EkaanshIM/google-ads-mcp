...
function isYesterdaySpendConversionQuestion(message) {
  const normalized = message.toLowerCase();
  const asksYesterday = normalized.includes("yesterday");
  const asksSpend = normalized.includes("spend") || normalized.includes("cost");
  const asksConversions = normalized.includes("conversion") || normalized.includes("conversions");
  return asksYesterday && asksSpend && asksConversions;
}

function rowsFromToolOutput(toolOut) {
  const content = toolOut?.content;
  if (!Array.isArray(content)) return Array.isArray(toolOut) ? toolOut : [toolOut].filter(Boolean);
...
function safeTimeZone(timeZone) {
  const normalized = timeZone === "Asia/Calcutta" ? "Asia/Kolkata" : timeZone;
  try {
...
function yesterdayDateInTimeZone(timeZone) {
  ...
  const yesterdayUtc = new Date(Date.UTC(today.year, today.month - 1, today.day) - 86_400_000);
  return ymdFromUtcDate(yesterdayUtc);
}

function readableDate(ymd, timeZone) {
...
async function runYesterdaySpendConversionFastPath(customerId) {
  ...
  const mcp = await getMcpClient();
  const customerOut = await mcp.callTool({
    name: "search",
    arguments: {
      customer_id: cid,
      resource: "customer",
      fields: ["customer.descriptive_name", "customer.currency_code", "customer.time_zone"],
      limit: 1
    }
  });

  const customerRow = rowsFromToolOutput(customerOut)[0] || {};
  const accountName = field(customerRow, "customer.descriptive_name") || "the account";
  const currency = field(customerRow, "customer.currency_code") || "INR";
  const timeZone = safeTimeZone(field(customerRow, "customer.time_zone") || process.env.GOOGLE_ADS_ACCOUNT_TIME_ZONE);
  const date = yesterdayDateInTimeZone(timeZone);

  const metricsOut = await mcp.callTool({
    name: "search",
    arguments: {
      customer_id: cid,
      resource: "customer",
      fields: ["customer.descriptive_name", "metrics.cost_micros", "metrics.conversions"],
      conditions: [`segments.date BETWEEN '${date}' AND '${date}'`],
      limit: 1
    }
  });

  const metricsRow = rowsFromToolOutput(metricsOut)[0] || {};
  const costMicros = Number(field(metricsRow, "metrics.cost_micros") || 0);
  const conversions = Number(field(metricsRow, "metrics.conversions") || 0);
  const spend = costMicros / 1_000_000;

  const lines = [
    `Yesterday (${readableDate(date, timeZone)}), the total spend and conversions for ${accountName} (customer ID: ${cid}) were:`,
    "",
    `- Total spend: ${formatCurrency(spend, currency)}`,
    `- Conversions: ${conversions.toLocaleString("en-IN", { maximumFractionDigits: 2 })}`
  ];
...
async function runChatJob(job) {
  if (isYesterdaySpendConversionQuestion(job.message)) {
    return runYesterdaySpendConversionFastPath(job.customerId);
  }
